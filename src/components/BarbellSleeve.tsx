...
import { eachSideCopy, plateById, plateColor, plateSize, type PlateStackItem, type PlateTheme } from '../engine';
...
export function BarbellSleeve({
  ...
  const rawSleeve = disks.reduce((sum, disk) => {
    ...
    return sum + (spec ? plateSize(spec).width + GAP : 0);
  }, 0);
  ...
  return (
    <View style={[styles.sleeve, side === 'left' ? styles.sleeveLeft : styles.sleeveRight]}>
      ...
      {disks.map((disk) => {
        ...
        const width = Math.max(10, Math.round(size.width * scale));
        const height = Math.max(28, Math.round(size.height * (0.82 + 0.18 * scale)));
        const text = label(disk.weight);
        ...
        return (
          ...
          >
            ...
            >
              ...
              />
              {fontSize > 0 ? (
                <Text style={[styles.badge, { color: color.text, fontSize }]} allowFontScaling={false}>
                  {text}
                </Text>
              ) : null}
            </Pressable>
...
function badgeFontSize(text: string, width: number): number {
  const needed =
    text.length >= 4 ? 36 : text.includes('.') ? 30 : text.length === 3 ? 28 : 18;
  if (width < needed) {
    return 0;
  }
  if (text.length >= 4) {
